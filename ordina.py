...
# Alcuni CDN (in particolare quelli di Warner Bros. Discovery Italia: Nove,
# Real Time, Food Network, Discovery Channel, DMAX, HGTV...) rifiutano le
# richieste che non hanno uno User-Agent "da browser" - per questo su TV/app
# IPTV spesso non si vedono pur funzionando da browser. La CDN esatta cambia
# nel tempo (osservato un cambio di dominio nel giro di pochi minuti), quindi
# invece di riconoscerla per firma applico l'header a tutti i canali: e'
# innocuo per RAI/Mediaset (non fanno controlli sullo User-Agent) e robusto
# a futuri cambi di infrastruttura lato Discovery.
AGGIUNGI_USER_AGENT = True
...
def main():
    ...
    for line in lines:
        ...
        if line.startswith("#EXTINF"):
            ...
            if key in LCN_TIVUSAT:
                ...
                cand = {"lcn": lcn, "extinf": ext, "url": line,
                        "is_hd": is_hd, "name": raw, "needs_ua": AGGIUNGI_USER_AGENT}
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        for c in ordinati + sconosciuti:   # sconosciuti (se presenti) in fondo, senza numero
            f.write(c["extinf"] + "\n")
            if c["needs_ua"]:
                f.write(f'#EXTVLCOPT:http-user-agent={BROWSER_UA}\n')
            f.write(c["url"] + "\n")

    print(f"Fatto: {len(ordinati)} canali numerati", end="")
    if sconosciuti:
        print(f" + {len(sconosciuti)} senza LCN in fondo", end="")
    if AGGIUNGI_USER_AGENT:
        print(f" (header User-Agent browser incluso su ogni canale)", end="")
    print(f" -> '{OUTPUT_FILE}'.")
